//! gRPC 服务适配器
//!
//! 实现 Task trait,使 gRPC 服务可以被 ServiceRuntime 管理
...
///
/// let adapter = GrpcAdapter::new("my-grpc", "0.0.0.0:8080".parse().unwrap(), |shutdown_rx| {
///     Server::builder()
...
impl<F, Fut> Task for GrpcAdapter<F, Fut>
where
    F: FnOnce(tokio::sync::oneshot::Receiver<()>) -> Fut + Send + 'static,
    Fut: Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send + 'static,
{
...
impl GrpcAdapterBuilder {
    ...
    /// 构建 gRPC 适配器
    pub fn build<F, Fut>(self, serve_fn: F) -> GrpcAdapter<F, Fut>
    where
        F: FnOnce(tokio::sync::oneshot::Receiver<()>) -> Fut + Send + 'static,
        Fut: Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send + 'static,
    {
...
mod tests {
    ...
    fn test_grpc_adapter_new() {
        ...
        assert_eq!(adapter.name(), "test-grpc");
        assert!(adapter.is_critical());
    }

    #[test]
    fn test_grpc_adapter_builder() {
        let adapter = GrpcAdapterBuilder::new("test-grpc", "0.0.0.0:8080".parse().unwrap())
