...
async fn example_quick_start() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    use crate::discovery::{BackendType, DiscoveryFactory, InstanceMetadata, ServiceInstance};
...
async fn example_service_registration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    use crate::discovery::{
        BackendType, DiscoveryConfig, DiscoveryFactory, InstanceMetadata, ServiceInstance,
    };
    use serde_json::json;
    use std::collections::HashMap;
    use std::net::SocketAddr;
...
async fn example_etcd_discovery() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    use crate::discovery::{BackendType, DiscoveryConfig, DiscoveryFactory, LoadBalanceStrategy};
    use serde_json::json;
    use std::collections::HashMap;
...
{
    use crate::discovery::{
        BackendType, DiscoveryConfig, DiscoveryFactory, LoadBalanceStrategy, NamespaceConfig,
        VersionConfig,
    };
    use serde_json::json;
    use std::collections::HashMap;
...
async fn example_with_tags() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    use crate::discovery::{
        BackendType, DiscoveryConfig, DiscoveryFactory, LoadBalanceStrategy, TagFilter,
    };
    use serde_json::json;
    use std::collections::HashMap;
