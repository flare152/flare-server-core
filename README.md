...
Flare IM 服务端基础库：统一运行时、gRPC/HTTP 适配、MQ 消费、错误与上下文、服务发现、认证等。与业务 proto 解耦，通过 feature 可选接入 `flare-proto`。

## 许可

[MIT License](LICENSE)。可自由使用、修改与再分发；分发时请保留版权声明与许可全文。详见 [NOTICE](NOTICE)。
...
- `AuthenticatedPrincipal` / `TokenValidationRequest` / `TokenValidator`：业务无关认证 provider 合同，用于 Gateway、BFF 或插件统一消费 token 校验结果和 scopes。
...
**版本**: 见根目录 `Cargo.toml`（当前 workspace `0.2.0`）
