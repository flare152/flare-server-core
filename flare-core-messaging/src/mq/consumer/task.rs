//! MQ 消费者与 `flare_core_runtime::task::Task` 的桥接
//!
//! 通用「拉取型」MQ 消费者实现 [MqConsumer]，经 [MqConsumerTask] 注册到 `ServiceRuntime`。
...
/// 在 `ServiceRuntime` 中运行的 MQ 消费者契约（JetStream / NATS 等）
...
/// 将 [MqConsumer] 包装为 [Task]，供 `ServiceRuntime::add_task` / `ServiceRuntime::add_mq_consumer` 使用
