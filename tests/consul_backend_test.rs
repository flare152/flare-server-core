//! Consul 后端集成测试
//!
//! 这些测试需要运行中的 Consul 服务器实例。
//! 默认情况下，测试会被忽略，需要使用 `cargo test --test consul_backend_test -- --ignored` 运行。
//!
//! 启动 Consul 服务器：
//! ```bash
//! # 使用 Docker 启动 Consul
...
/// Consul 服务器地址
/// 可以通过环境变量 CONSUL_URL 覆盖，默认为 http://127.0.0.1:8500
fn consul_url() -> String {
    std::env::var("CONSUL_URL").unwrap_or_else(|_| "http://127.0.0.1:28500".to_string())
}
...
fn create_test_config(service_type: &str) -> DiscoveryConfig {
    ...
    backend_config.insert("url".to_string(), json!(consul_url()));
...
#[tokio::test]
#[ignore]
async fn test_consul_register() {
    // 设置环境变量，跳过健康检查（使用 TTL 检查）
    unsafe {
        std::env::set_var("CONSUL_SKIP_HEALTH_CHECK", "1");
        std::env::set_var("CONSUL_PASSING_ONLY", "false");
    }
    
    let config = create_test_config("test-service");
    let backend = DiscoveryFactory::create_backend(&config)
        .await
        .expect("Failed to create consul backend");
    ...
    assert!(
        ...
        "Service instance not found after registration. Found instances: {:?}",
    ...
    // 清理环境变量
    unsafe {
        std::env::remove_var("CONSUL_SKIP_HEALTH_CHECK");
        std::env::remove_var("CONSUL_PASSING_ONLY");
    }
}
...
async fn test_consul_discover() {
    ...
    assert!(
        instances.len() >= 2,
        "Expected at least 2 instances, found {}. Found: {:?}",
        instances.len(),
        instances.iter().map(|i| &i.instance_id).collect::<Vec<_>>()
    );
    assert!(
...
#[tokio::test]
#[ignore]
async fn test_consul_service_discover() {
    // 设置环境变量，跳过健康检查（使用 TTL 检查）
    unsafe {
        std::env::set_var("CONSUL_SKIP_HEALTH_CHECK", "1");
        std::env::set_var("CONSUL_PASSING_ONLY", "false");
...
#[tokio::test]
#[ignore]
async fn test_consul_heartbeat() {
...
#[tokio::test]
#[ignore]
async fn test_consul_create_with_defaults() {
    // 使用默认配置创建服务发现
...
#[tokio::test]
#[ignore]
async fn test_consul_tag_filter() {
    // 设置环境变量，跳过健康检查（使用 TTL 检查）
    unsafe {
        std::env::set_var("CONSUL_SKIP_HEALTH_CHECK", "1");
        std::env::set_var("CONSUL_PASSING_ONLY", "false");
...
async fn test_consul_health_check() {
    ...
    // 注册服务实例
    let instance = create_test_instance("test-service", "node-10", 8089);
    backend
        .register(instance.clone())
        .await
        .expect("Failed to register service");
    ...
    // 使用 ServiceClient 获取 Channel（应该自动跳过不健康的节点）
    let mut client = ServiceClient::new(discover);
    ...
    match channel_result {
        Ok(Ok(_channel)) => {
            println!("✅ Successfully obtained channel from healthy service");
        }
        Ok(Err(e)) => {
            println!("⚠️ Failed to get channel: {}", e);
        }
        Err(_) => {
            println!("⚠️ Timeout waiting for channel (no healthy services available)");
        }
    }
